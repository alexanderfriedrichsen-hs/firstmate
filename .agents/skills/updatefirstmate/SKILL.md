...
Firstmate is its own repo, behind the same no-mistakes gate as any project, so new tracked material (`AGENTS.md`, `bin/`, `.agents/skills/`, public `skills/`, and the localhost app's `app/`, `package.json`, `package-lock.json`, `tsconfig.json`) reaches `main` and then sits there until each running firstmate pulls it.
Only `AGENTS.md`, `bin/`, and `.agents/skills/` are a running firstmate instruction surface; public `skills/` is installer-facing and is not loaded by firstmate, and the app is a separate opt-in program that this fast-forward keeps current but does not restart.
